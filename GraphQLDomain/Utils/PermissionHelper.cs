...
namespace GraphQLDomain.Utils
{
    public static class PermissionHelper
    {
        ...
        public static bool RequirePermissions(this IProvideMetadata metadata)
        {
            var permissions = metadata.GetMetadata<IEnumerable<string>>(PermissionsKey, new List<string>());
        ...
        {
            var permissions = type.GetMetadata<IEnumerable<string>>(PermissionsKey, new List<string>());
            return permissions.All(x => claims?.Select(x =>x.Value).Contains(x) ?? false);
        }
